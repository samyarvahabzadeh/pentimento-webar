...
async function initializeAR() {
  ...
  try {
    ...
    // Build A-Frame assets and model markup based on fallback setting
    let assetsHTML = '';
    let modelHTML = '';
    
    const scaleStr = `${CONFIG.arObject.scale.x} ${CONFIG.arObject.scale.y} ${CONFIG.arObject.scale.z}`;
    ...
    if (CONFIG.useFallbackModel) {
      ...
      assetsHTML = `
        <a-assets>
          <a-asset-item id="coffee-cup-model" src="${CONFIG.modelPath}"></a-asset-item>
        </a-assets>
      `;
      modelHTML = `
        <a-entity id="cafe-scene-container" rotation="${rotStr}" position="${posStr}" scale="${scaleStr}">
          <a-gltf-model src="#coffee-cup-model"></a-gltf-model>
          
          <!-- Steam Particles above the GLB model -->
          <a-entity id="steam-container">
            <a-sphere color="#ffffff" radius="0.008" opacity="0.15" position="-0.02 0.26 0.01" animation="property: position; to: -0.01 0.41 0.02; dur: 2500; loop: true; easing: linear;"></a-sphere>
            <a-sphere color="#ffffff" radius="0.008" opacity="0.15" position="0.02 0.26 -0.01" animation="property: position; to: 0.01 0.38 -0.02; dur: 2000; loop: true; easing: linear;"></a-sphere>
            <a-sphere color="#ffffff" radius="0.008" opacity="0.15" position="0 0.28 0" animation="property: position; to: 0 0.44 0.01; dur: 3000; loop: true; easing: linear;"></a-sphere>
    ...
    const sceneHTML = `
      <a-scene id="ar-scene" mindar-image="imageTargetSrc: ${targetPath}; autoStart: true; uiLoading: no; uiScanning: no; filterMinCF: ${CONFIG.mindAR.filterMinCF}; filterBeta: ${CONFIG.mindAR.filterBeta}; missTolerance: ${CONFIG.mindAR.missTolerance}; warmupTolerance: ${CONFIG.mindAR.warmupTolerance};" color-space="sRGB" renderer="colorManagement: true, physicallyCorrectLights" vr-mode-ui="enabled: false" device-orientation-permission-ui="enabled: false">
        ${assetsHTML}
